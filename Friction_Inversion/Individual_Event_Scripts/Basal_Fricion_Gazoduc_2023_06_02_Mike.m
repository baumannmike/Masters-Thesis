...
% [cross_sectional_area,width,depth,depth_mean] = get_cross_section_area_width(...
...
%      vel_cutoff,...
%     depth_cutoff);
% % [depth_rm_outliers,idx] = rmoutliers(depth_mean(:,interesting_sections),"movmedian",50,ThresholdFactor=1);
% % depth_rm_outliers = depth_mean(:,target_section);
% % depth_rm_outliers(idx) = NaN;

%% cross sectional area, width, depth, remove outliers
...
good_ind_dz_dy=-0.4;
good_ind_dv2_dy_max=0.3;
good_ind_dv2_dy_min= -0.105;
good_ind_dv_dy_max=-0.504;%-0.07;
...
dvdt_thresh = 0.5;   % <- musst du evtl. anpassen!
...
index = 5;
% figure
t0 = [datetime(2023,04,28,23,27,49,352) datetime(2023,06,02,19,49,10,150) datetime(2023,07,12,05,31,17,1273) datetime(2023,07,13,01,31,35,18)  datetime(2023,11,14,17,41,40)];
